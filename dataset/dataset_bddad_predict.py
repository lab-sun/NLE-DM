...
class BDD_AD(Dataset):
    def __init__(self, Root, cropSize=(1280, 720), train_set=False):
        ...
        if train_set:
            ...
            self.imgroot = os.path.join(self.root, 'test_2500')
            with open(os.path.join(self.root, "test_action.json")) as json_file:
                action = json.load(json_file)
            with open(os.path.join(self.root, "test_des.json")) as json_file:
